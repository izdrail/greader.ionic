...
export class SubscribePage implements OnInit { mode:'url'|'browse'|'news'|'podcasts'|'opml'='url';url='';query='';busy=signal(false);importProgress=signal('');pending=signal('');accountId='';bundles=signal<DirectoryBundle[]>([]);podcasts=signal<PodcastResult[]>([]);recommendations=signal<RecommendedFeed[]>([]);topics=['news','technology','science','business','php','laravel','design'];keywords=signal<string[]>([]);filtered=signal<DirectoryBundle[]>([]);
...
async switchMode(mode:typeof this.mode){this.mode=mode;this.query='';this.podcasts.set([]);this.loadError.set('');if(mode==='browse'||mode==='news'){this.recommendations.set([]);await this.loadDirectory();}if(mode==='podcasts'&&!this.keywords().length)this.keywords.set(await this.directory.podcastKeywords());}
...
async discoverTopic(value=this.query){const topic=value.trim();if(!topic)return;this.query=topic;this.busy.set(true);this.loadError.set('');try{const rows=await this.directory.recommendations(topic);this.recommendations.set(rows);if(!rows.length)this.loadError.set(`No feeds found for "${topic}". Try a broader topic.`);}catch(e){this.loadError.set(this.message(e));await this.toast(this.message(e),'danger');}finally{this.busy.set(false);}}
