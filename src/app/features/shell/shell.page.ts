...
import { addIcons } from 'ionicons'; import { addOutline,browsersOutline,checkmarkDoneOutline,cloudOfflineOutline,downloadOutline,gridOutline,listOutline,mailOpenOutline,mailUnreadOutline,menuOutline,refreshOutline,settingsOutline,star,starOutline,volumeHighOutline } from 'ionicons/icons';
import { StoragePort } from '../../core/storage/storage.port'; import { Account,Article,Subscription } from '../../core/domain/models'; import { ARTICLE_LIST_MODES, MARK_READ_AGES, sortSubscriptions } from '../../core/domain/list-preferences'; import { ListPreferencesService } from '../../core/services/list-preferences.service'; import { ArticleActionsService } from '../../core/services/article-actions.service'; import { ThemeService } from '../../core/theme/theme.service';
...
export class ShellPage implements OnInit {
  accounts=signal<Account[]>([]);subscriptions=signal<Subscription[]>([]);articles=signal<Article[]>([]);query=signal('');filter=signal<'all'|'unread'|'starred'>('all');
  ...
  sortedSubscriptions=computed(()=>sortSubscriptions(this.subscriptions(),this.prefs.feedSort()));
  ...
  constructor(private db:StoragePort,private themes:ThemeService,private actions:ArticleActionsService,private alerts:AlertController,private toasts:ToastController,public prefs:ListPreferencesService){
    addIcons({menuOutline,refreshOutline,settingsOutline,listOutline,gridOutline,browsersOutline,downloadOutline,volumeHighOutline,star,starOutline,cloudOfflineOutline,addOutline,checkmarkDoneOutline,mailOpenOutline,mailUnreadOutline});
  }
  ...
  async load(){
    ...
    if(a[0]){
      const [subs,articles]=await Promise.all([this.db.listSubscriptions(a[0].id),this.db.listArticles(a[0].id)]);
